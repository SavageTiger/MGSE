...
function doAltTab(shellwm, binding, window, backwards) {
    ...
    if(settings.get_boolean(SETTINGS_FIRST_TIME_KEY)) {        
        // Show Workspace icons by default - other modes don't work well and the question shouldn't be thrown at the user's face. 
        // This is configurable by the user via gsettings and it should be configurable graphically via something like mintDesktop in the future
        settings.set_string(SETTINGS_BEHAVIOUR_KEY, "workspace_icons");
        settings.set_boolean(SETTINGS_FIRST_TIME_KEY, false);
        MODES["workspace_icons"](binding, backwards);
